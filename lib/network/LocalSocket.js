...
class LocalSocket extends EventEmitter {
    ...
    async startSocket() {
        ...
        if (this._localPeerAddress.isSSL()) {
            ...
            options.key = fs.readFileSync(Settings.network.ssl.key);
            options.cert = fs.readFileSync(Settings.network.ssl.cert);
    ...
    static async getLocalAddressAvailable() {
        ...
        if (allowPortIncrementation) {
            ...
            return new PeerAddress({
                ip: Settings.network.peer.ip || null,
                host: Settings.network.peer.host || null,
                ssl: Settings.network.ssl ? true : false,
                port: port
            });
