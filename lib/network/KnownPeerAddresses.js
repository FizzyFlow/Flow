...
class KnownPeerAddresses extends EventEmitter {
    ...
    getDiscoveryResponse(fromTimestamp) {
        ...
        resp.addresses = this._addresses.filter((peerAddress)=> {
            if (resp.timestamp < this._timestamps.get(peerAddress.id)) {
                resp.timestamp = this._timestamps.get(peerAddress.id);
            }
            return (
                this._timestamps.get(peerAddress.id) > fromTimestamp &&
                this._statuses.get(peerAddress.id).status != PeerStatus.BANNED
                );
        });
    ...
    getPeerAddressStatus(peerAddress) {
        ...
        return this._statuses.get(peerAddress.id).status;
    }
